...
/**
 * Guards the single control that keeps signature OTP codes out of HTTP responses
 * on a deployed profile. If any of these break, the fail-fast protection is gone.
 */
class OtpExposureGuardTest {
    ...
    private OtpExposureGuard guard(boolean expose, String... profiles) {
        ...
        ReflectionTestUtils.setField(guard, "exposeOtp", expose);
        return guard;
    }
