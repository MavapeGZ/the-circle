...
export default function ReviewForm({ targetUserId, contractId, onSubmitted, onCancel }) {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (rating < 1) {
      setError(t('reviewForm.selectRating'));
      return;
    }
    setSubmitting(true);
    setError('');
    try {
      ...
      setError(extractApiError(err, t('reviewForm.submitError')));
      setSubmitting(false);
    }
  };
  ...
  return (
    <form onSubmit={submit} className="mt-4 p-4 bg-gray-50 border border-gray-200 rounded-xl space-y-3">
      <h3 className="font-bold text-gray-800">{t('reviewForm.title')}</h3>
      <div className="flex items-center gap-3">
        <StarRating value={rating} onChange={setRating} size={28} />
        <span className="text-sm text-gray-500">{rating ? t('reviewForm.ratingValue', { rating }) : t('reviewForm.tapToRate')}</span>
      </div>
      <textarea
        value={comment}
